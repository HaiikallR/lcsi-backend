...
View dari Blade Component <x-layout.sidebar>
    ...
    --}}
    <aside class="flex flex-col bg-slate-900 text-white transition-all duration-300 ease-in-out flex-shrink-0"
        :class="sidebarOpen ? 'w-64' : 'w-16'" style="font-family: 'DM Sans', sans-serif;">
        {{-- Logo --}}
        <div class="flex items-center gap-3 px-4 py-5 border-b border-slate-700/50">
            <div
                class="w-9 h-9 bg-blue-500 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-blue-500/30">
            ...
            <div x-show="sidebarOpen" x-transition:enter="transition-opacity duration-200"
                ...
                <p class="font-display font-bold text-base leading-tight" style="font-family: 'Sora', sans-serif;">LCSI
                </p>
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-1">
            @foreach($menus as $group)
                ...
                @foreach($group['items'] as $item)
                    ...
                    @endphp
                    <a href="{{ route($item['route']) }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150 group relative
                             {{ $isActive }}
                        ...
                        {{-- Label --}}
                        <span x-show="sidebarOpen" class="text-sm font-medium truncate"
        ...
        <div class="border-t border-slate-700/50 p-3">
            <div class="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-slate-800 transition">
                <div
                    class="w-8 h-8 bg-blue-500/20 border border-blue-500/30 rounded-lg flex items-center justify-center flex-shrink-0">
