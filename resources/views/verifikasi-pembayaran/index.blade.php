...
@section('content')
    ...
    <x-ui.page-header title="Verifikasi Pembayaran" :sub="'Total ' . $verifikasis->total() . ' pengajuan'" />
                    ...
                    @forelse($verifikasis as $v)
                        <tr class="hover:bg-slate-50/40 transition">
                            ...
                            <td class="px-5 py-3.5">
                                ...
                                @if($v->bukti_bayar)
                                    <a href="{{ Storage::url($v->bukti_bayar) }}" target="_blank" class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 text-blue-600
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if($v->status === 'menunggu')
                                        <form method="POST" action="{{ route('verifikasi-pembayaran.setujui', $v) }}" x-data
                                            @submit.prevent="if(confirm('Setujui pembayaran ini?')) $el.submit()">
                                            @csrf
                                            <button type="submit"
                                    ...
                                    {{-- Tombol Hapus --}}
                                    <form method="POST" action="{{ route('verifikasi-pembayaran.destroy', $v) }}" x-data
                                        @submit.prevent="if(confirm('Hapus data verifikasi ini?')) $el.submit()">
                                        @csrf @method('DELETE')
                                        <button type="submit"
