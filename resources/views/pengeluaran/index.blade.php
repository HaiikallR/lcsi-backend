...
@section('content')
    ...
    <x-ui.page-header title="Manajemen Pengeluaran" :sub="'Total ' . $pengeluarans->total() . ' data pengeluaran'"
        create-route="pengeluaran.create" create-label="Tambah Pengeluaran" />
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50/70 border-b border-slate-100">
                        ...
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase">Kategori
                            Pengeluaran
                        </th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($pengeluarans as $p)
                        <tr class="hover:bg-slate-50/40 transition">
                            ...
                            <td class="px-5 py-3.5 font-medium text-slate-700">Rp {{ number_format($p->jumlah, 0, ',', '.') }}
                            </td>
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('pengeluaran.destroy', $p) }}" x-data
                                        @submit.prevent="if(confirm('Hapus data pengeluaran ini?')) $el.submit()">
                                        @csrf @method('DELETE')
                                        <button type="submit"
