...
@section('content')
    <x-ui.page-header title="Manajemen Pemasukan" :sub="'Total ' . $pemasukans->total() . ' data'"
        create-route="pemasukan.create" create-label="Tambah Pemasukan" />
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50/70 border-b border-slate-100">
                        ...
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase">Bukti Pembayaran
                        </th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($pemasukans as $p)
                        <tr class="hover:bg-slate-50/40 transition">
                            ...
                            <td class="px-5 py-3.5 font-medium text-emerald-700">Rp
                                ...
                                @if($p->bukti_bayar)
                                    <div class="flex flex-col gap-1">
                                        <a href="{{ asset('storage/' . $p->bukti_bayar) }}" target="_blank" class="group relative">
                                            <img src="{{ asset('storage/' . $p->bukti_bayar) }}" alt="Bukti"
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('pemasukan.destroy', $p) }}" x-data
                                        @submit.prevent="if(confirm('Hapus data ini?')) $el.submit()">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-slate-300 text-sm">Belum ada data pemasukan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-5 py-4 border-t border-slate-50">{{ $pemasukans->links() }}</div>@endif
    </div>
@endsection