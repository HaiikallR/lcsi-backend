...
@section('content')
    ...
    <x-ui.page-header title="Manajemen Tiket" :sub="'Total ' . $tikets->total() . ' tiket'" create-route="tiket.create"
        create-label="Buat Tiket" />
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50/70 border-b border-slate-100">
                        ...
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase">Pelanggan/Calon
                        </th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase">Jenis Pekerjaan
                        </th>
                ...
                <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/40 transition">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if($t->teknisi?->no_hp)
                                        <a href="{{ route('tiket.whatsapp', $t) }}" target="_blank"
                                    ...
                                    <form method="POST" action="{{ route('tiket.destroy', $t) }}" x-data
                                        @submit.prevent="if(confirm('Hapus tiket #{{ $t->id }}?')) $el.submit()">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-slate-300 text-sm">Belum ada tiket</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
