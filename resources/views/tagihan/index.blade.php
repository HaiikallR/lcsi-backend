...
@section('content')
    ...
    <x-ui.page-header title="Manajemen Tagihan" :sub="'Total ' . $tagihans->total() . ' tagihan'"
        create-route="tagihan.create" create-label="Buat Tagihan Khusus" />
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($tagihans as $t)
                        <tr class="hover:bg-slate-50/40 transition">
                            ...
                            <td class="px-5 py-3.5 font-medium text-slate-700">Rp {{ number_format($t->jumlah, 0, ',', '.') }}
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('tagihan.destroy', $t) }}" x-data
                                        @submit.prevent="if(confirm('Hapus tagihan ini?')) $el.submit()">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center text-slate-300 text-sm">Belum ada tagihan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
